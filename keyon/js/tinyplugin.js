$(function()
{
                ...
                .done(function(data){
                  StopPreloader();
                    ...
                    if(jdata.result=="Inserted")
                    {
                      ...
                      //now lets check orderobj to see if a nurse sent the order and if so lets send out the notification
                      if(orderobj.OrderTemp.verbalorder=="true")
                      {
                        //lets grap Physicians Name and NPi number and then resend the OrderObje with an Updated API_Meth
                        
                        alert("A notification for Order Number"+" "+ orderobj.ordernumber +" "+"to be signed has been sent to"+" "+ orderobj.OrderTemp.primphysician+".");
                      }
                    }
                });
  ...
  $("#cancelbtn").click(function(e){
   ...
   //event for Adding Provider
    ...
    $(".tabs #medicationbtn").click(function(e){
        ...
        .done(function(data){
          StopPreloader();
            ...
            $("#medicationcanvas .order-grid").find(".columnbottom").html('');
            $("#medicationcanvas .order-grid").find(".columnbottom").append(jdata.gridhtml);// .columntop").append(jdata.gridhtml);
      ...
      //MEdication Delete Button Click Event 
      $(".medbtn_nav").find("#delettransbtn").click(function(e){
         console.log("Delete Button Clicked");
         console.log(medobj.medentryid);
         //lets make sure a record was selected
         if(medobj.medentryid && medobj.medentryid !=undefined)
         {


             let ask = confirm("Are you sure you want to remove Medical Entry"+" "+medobj.medentryid+"?");
             if(ask==true)
             {
              /*now do work 
              @ medobj loops - Purpose (clean up medobj Object so the Applicable Server side logic will be interperted correctly)
              */
              ...
              medobj.MedicationUpdate ={};
              medobj.MedicationUpdate.API_Meth="DeleteMedication";
              medobj.MedicationUpdate.medentryid = medobj.medentryid;
              medobj.MedicationUpdate.patientid="45064367";
              ShowPreloader();
               ...
               .done(function(data){
                ...
                {
                  alert("Medical Entry #"+" "+medobj.medentryid+" "+"was successfully deleted");
                  /*Now Repopulate MedGrid so records are updated*/
                  PopuplateMedGrid("45064367");
                }
              });
             }
             else{
             ...
             alert("No Record was selected from the Medication Grid, please select/click a record in order to remove an Medication record.");
           }
      ...
      $(".tabs #diagnosisbtn").click(function(e){
        ...
        .done(function(data){
           StopPreloader();
            ...
            $("#diagnosiscanvas .order-grid").find(".columnbottom").html('');
            $("#diagnosiscanvas .order-grid").find(".columnbottom").append(jdata.gridhtml);
            $("#diagnosiscanvas .columnrow").click(function(e){
         //get the Id number of the column 
            e.preventDefault();
         
             let diagId = $(this).attr("data-diagid");
         
        // let diagId = $(this).find(".columnrow").data("diagid");
         ...
         // StopPreloader();
            ...
            if(jdata.data[0].diagnosis_type=="Diagnosis")
            {
                ...
                $("#procedurechkbox").attr("checked",false);
                $("#cicd10thirdRating").css("display","block");
                $("#procedurelookupterms").css("display","none");
                $("#srchnlookupterms").css("display","block");
                $("#srchnlookupterms").val(jdata.data[0].diagnosis_code);
                $(".procorder-grid").css("display","none");
            }
            if(jdata.data[0].diagnosis_type=="Procedure")
            {

               $("#procedurechkbox").attr("checked",true);
               $("#diagchkbox").attr("checked",false)
               //hide the system control element
               $("#icd10thirdRating").css("display","none");
               $("#srchnlookupterms").css("display","none");
               $("#procedurelookupterms").css("display","block");
               $("#procedurelookupterms").val(jdata.data[0].procedurecode);
      ...
      $(".diagnosisbtn_nav").find("#delettransbtn").click(function(e){
         console.log("Diagnosis Delete Button Clicked");
         console.log(diagobj);
       e.preventDefault();
          diagobj.Diagnosis.API_Meth="DeleteDiagnosis";
         //lets make sure a record was selected
         if(diagobj.Diagnosis.diagID && diagobj.Diagnosis.diagID !=undefined)
         {


             let ask = confirm("Are you sure you want to remove Diagnosis Entry"+" "+diagobj.Diagnosis.diagID+"?");
             if(ask==true)
             {
              /*now do work 
              @ medobj loops - Purpose (clean up medobj Object so the Applicable Server side logic will be interperted correctly)
              */
              ClearObjects("DeleteDiagnosis");
                /*for (var member in medobj.MedGrid) delete medobj.MedGrid[member];
               for (var member in medobj.GetCurrentMed) delete medobj.GetCurrentMed[member];
               for (var member in obj) delete obj[member];*/
          
             // diagobj.UpdateDiagnosis ={};
              diagobj.Diagnosis.API_Meth="DeleteDiagnosis";
              //diagID should property should already be in place because of the grid click 
               diagobj.Diagnosis.patientid="45064367";
              ShowPreloader();
               $.post(postweburl,JSON.stringify(diagobj))
               .done(function(data){
                ...
                let jdata = JSON.parse(data);
                console.log(jdata);
                if(jdata.result=="Deleted")
                {
                  alert("Diagnosis Entry #"+" "+diagobj.Diagnosis.diagID+" "+"was successfully deleted");
                  /*Now Repopulate MedGrid so records are updated*/
                  PopuplateDiagGrid("45064367");
                }
              });
             }
             else{
             ...
             alert("No Record was selected from the Diagnosis Grid, please select/click a record in order to remove a Diagnosis Entry record.");
           }
      ...
      $("#icdsfrthopt").find("#savediagnbtn").click(function(e){
         ...
         else if(diagobj.Diagnosis.API_Meth=="InsertDiagnosis") 
         {
            console.log(diagobj);
            ...
            .done(function(data){
              StopPreloader();
                let jdata = JSON.parse(data);
                if(jdata.Type=="Diagnosis" && jdata.result =="Inserted")
                {
        ...
        else if(diagobj.Diagnosis.API_Meth=="SearchProcedure-Code" || diagobj.Diagnosis.API_Meth=="SearchProcedure-Description")
        {
           ClearObjects("InsertDiagnosis");//need to update the object to procedure 
           if($("#procedurechkbox").prop("checked")==true && $("#diagchkbox").prop("checked")==false)
           {
            diagobj.Diagnosis.diagnosis_type="Procedure";
            //now update the diagnosis Object API_Method to ProcedureInsert
            diagobj.Diagnosis.API_Meth="ProcedureInsert";
             if($("#onsetkbox").prop("checked"))
                {
                    diagobj.Diagnosis.onset_exacerbation="onset";
                }
            ...
            diagobj.Diagnosis.diagnosisDate = $("#diagnosisDate").val();
            diagobj.Diagnosis.procedurecode = $("#procedurelookupterms").val();
              ShowPreloader();
            $.post(postweburl,JSON.stringify(diagobj))
            .done(function(data){
              StopPreloader();
                let jdata = JSON.parse(data);
                if(jdata.Type=="Procedure" && jdata.result =="Inserted")
                {
                    alert("Procedure Codes Information Inerted Succesfully");
                }
            });
     ...
     /* $("#addtransbtn").click(function(e){
      ...
      }); commenting this out for the new Add Transaction button 2/11/2023*/
      ...
      /*Adding new Code Here */
       $(".diagnosismodal").find("#procedurechkbox").change(function(e){
        e.preventDefault();
        if($("#procedurechkbox").prop("checked"))
        {
            $("#diagchkbox").attr("checked",false);
            //now change the UI
            diagobj.Diagnosis.Type="Procedure";
             /* When the toggle switch changes check against Type  and then search by Procedure or Diagnosis Type accordingly*/
            diagobj.Diagnosis.API_Meth="SearchProcedure-Code";

            $("#srchnlookupterms").css("display","none");
            $("#srchnlookupterms").next("a img").css("display","none");
            $("#icd10thirdRating").css("display","none");
            $("#procedurelookupterms").css("display","block");
            $("#srchbylbl").html("Search IC10 Procedure Codes");
            $(".procorder-grid").css("display","block");
        }
      });
      $(".diagnosismodal").find("#diagchkbox").change(function(e){
          //e.preventDefault();
          if($("#diagchkbox").prop("checked")==true && $("#procedurechkbox").prop("checked")==true)
          {
         ...
         // $("#procedurechkbox").attr("checked",false);
          ...
          $("#srchnlookupterms").css("display","block");
          $("#srchnlookupterm").next(" a img").css("display","none");
          $("#icd10thirdRating").css("display","block");//control
          $("#procedurelookupterms").css("display","none");
          $("#srchbylbl").html("Search IC10 Diagnosis Codes");
          $(".procorder-grid").css("display","none");
      });
      $("#procedurelookupterms").keyup(function(e){
       
        let switchback="";
           if(diagobj.Diagnosis.API_Meth && diagobj.Diagnosis.API_Meth =="EditDiagnosis")
          {
             switchback="EditDiagnosis";
          } 
          diagobj.Diagnosis.API_Meth="SearchProcedure-Code";
          diagobj.Diagnosis.procedurecode=$("#procedurelookupterms").val();
        let searchTerms = $("#procedurelookupterms").val();
        setTimeout(SetTimedPost(searchTerms,switchback),30000000);
      });
      function SetTimedPost(searchvalue,switchback)
      {
        diagobj.Diagnosis.searchterms=searchvalue;
        ShowPreloader();
        $.post(postweburl,JSON.stringify(diagobj))
        .done(function(data){
           if(switchback=="EditDiagnosis")
          {
            diagobj.Diagnosis.API_Meth="EditDiagnosis";
          } 
            StopPreloader();
            ...
            let jdata = JSON.parse(data);
            console.log(jdata);
            $(".diagnosismodal").css("height","872px");
            $(".procorder-grid").css("display","block");
            $(".icd10proceduresec .procorder-grid").find(".columnbottom").html('');
            $(".icd10proceduresec .procorder-grid").find(".columnbottom").append(jdata.html);
            //let create click event for the grid
            $(".procorder-grid").find(".columnbottom .columnrow").click(function(e){
                e.preventDefault();
                //remove any border style
                $(".procorder-grid .columnbottom").each(function(){
                    $(".columnrow").css("border-style","none");
                });
                $(this).find(".columnbottom").next(".columnrow").css("border-style","dotted");
                $(this).find(".columnbottom").next(".columnrow").css("border-width","1px");
               console.log("Proc Grid Clicked");
                
                //changed the border to dotted so there is a visual indicator that the grid item was selected 
                $(this).css("border-style","dotted");
                let procid = $(this).find("#procid").attr("procid");
                let proccode = $(this).find("#proccode").html();
                let ordnumber = glblordnum;
                diagobj.Diagnosis.procedureid=procid;
                diagobj.Diagnosis.ordernumber = ordnumber;
                $("#procedurelookupterms").val(proccode);
               //now close the grid
               $(".procorder-grid").css("display","none");
                console.log(diagobj.procedureid);
               /* $(this).css("border-style","dotted");
                let medid = $(this).find("#medid").attr("medid");
                let ordnumber = $(this).find("#medid").html();*/
            });
      ...
      $(".diagnosismodal #toggleSearch").change(function(e){
         ...
         if($(".diagnosismodal #toggleSearch").is(":checked"))
         {
            ...
            //lets checked to see if its diagnosis or procedure and do different function 
            if($("#procedurechkbox").prop("checked")==true && $("#diagchkbox").prop("checked") ==false) 
            {
                //now check toggle 
                
                    //search Procedure Terms b Description
                    //change labels 
                  
                    $("#icdfirstopt #srchbylbl").html("Search PCS Codes By Description");
                    $("#proceduresearchterms").css("placeholder","Search PCS By Description");
                    diagobj.Diagnosis.API_Meth="SearchProcedure-Description";
                    ...
                    //if its diagnosis then do below: 
                    $("#icdfirstopt #srchbylbl").html('Search ICD-10 Description');
                    $(".diagnosismodal #toggleSearch").attr("checked",true);
           ...
           if(!$(".diagnosismodal #toggleSearch").is(":checked"))
           {
                if($("#procedurechkbox").prop("checked")==true && $("#diagchkbox").prop("checked") ==false && $("#toggleSearch").prop("checked")==false)
                {
                   
                    $("#icdfirstopt #srchbylbl").html("Search By PCS Codes");
                    $("#proceduresearchterms").css("placeholder","Search PCS By Code");
                    diagobj.Diagnosis.API_Meth="SearchProcedure-Code";
                }
                else{
                    $("#srchbylbl").html('Search By ICD-10 Codes');
     ...
     function PopuplateDiagGrid(patientid)
     {
      let pid = patientid;
         diagobj.Diagnosis.patientID=pid;
        diagobj.Diagnosis.API_Meth="PopulateDiagGrid";
        let url="https://willowbuilt.it/fora/tswebook.php";
        ShowPreloader();
        $.post(url,JSON.stringify(diagobj))
        .done(function(data){
          StopPreloader();
            console.log(data);
            let jdata = JSON.parse(data);
        ...
        * Grab ID and send to server to get info and then display in Med UI*/
        $("#diagnosiscanvas .columnrow").click(function(e){
            ...
            console.log("grid instance clicked");
            //removed all dotted lines
            $("#diagnosiscanvas .columnrow").each(function(){
                $("#diagnosiscanvas .columnrow").css("border-style","none");
            });
            //changed the border to dotted so there is a visual indicator that the grid item was selected 
            $(this).css("border-style","dotted");
            let diagId = $(this).attr("data-diagid");
            let ordnumber = $(this).find(".columnrow").html();
            diagobj.diagID=diagId;
            diagobj.ordernumber = ordnumber;
            console.log(diagobj.diagId);
            //alert(medid);
    ...
    function PopuplateMedGrid(patientid)
     {
      let pid = patientid;
        medobj.MedGrid = {};
        medobj.MedGrid.pid=pid;
       ...
       // medobj.MedGrid.API_Meth="PopulateMedGrid";
        ...
        .done(function(data){
          StopPreloader();
            ...
            console.log(jdata.gridhtml);
            $("#medicationcanvas .order-grid").find(".columnbottom").html('');
            $("#medicationcanvas .order-grid").find(".columnbottom").append(jdata.gridhtml);
            /*now lets sets the event listeners here since the buttons are apart of the medcanvas
        * Grab ID and send to server to get info and then display in Med UI*/
        $("#medicationcanvas .columnrow").click(function(e){
            e.preventDefault();
